...
Если мы хотим предложить какому-то разработчику помощь в его проекте и внести какие-то изменения в него мы должны сделать pull request. Для этого необходимо:
1. На странице интересующего проекта на GitHub сделать fork проекта, ввести название и какой-либо комментарий.
2. Скопировать ссылку данного проекта
3. С помощью команды:

    git clone "скопированная ссылка на проект" (без кавычек)

   клонировать удаленный репозиторий себе на устройство. 
4. Чтобы наши изменения отслеживались необходимо перейти в клонированную директорию с помощью команды:

    cd "Имя директории" (без кавычек)

5. В этой директории мы создаем новую ветку и переходим в неё, далее работа будет проходить в ней

6. После внесения и фиксации изменений в локальном репозитории необходимо "запушить" их на свой fork интересующего проекта. Далее терминал подскажет какую команду нужно ввести.
7. Когда все необходимые изменения будут "запушены" в ваш fork, нужно перейти на странице форка в вашу ветку и нажать кнопку Create Pull Request и при необходимости ввести какие-то комментарии.
